...
type CompanyAging = { company: string; agingDays: number; status: string };
type CompanyAmount = { company: string; amountINR: number; status: string };

type DisputeChartData = {
  fyndVsOther: {
...
const OB_PALETTE = [
  "#F15A29", "#1C1C1E", "#D97706", "#3D1A0A", "#FF7043",
...
function DisputeTrackerSection({ data }: { data?: DisputeChartData }) {
  if (!data) {
    return (
      <div className="lc-card" style={{ textAlign: "center", padding: "2rem", color: "#9aa0ab" }}>
        Loading Dispute & Litigation Tracker data…
      </div>
    );
  }
  ...
  const doughnutOpts: any = {
    ...
    plugins: {
      ...
      datalabels: {
        color: "#fff", font: { size: 11, weight: "700" as const },
        formatter: (v: number, ctx: any) => {
          const t = ctx.chart.data.datasets[0].data.reduce((a: number, b: number) => a + b, 0);
          const p = Math.round(v / t * 100);
          return p >= 8 ? `${v}\n${p}%` : "";
        },
        anchor: "center", align: "center", textAlign: "center" as const,
      },
    },
  };
  ...
  const hBarOpts: any = {
    ...
    plugins: {
      ...
      datalabels: {
        ...
        color: "#1C1C1E", font: { size: 11, weight: "700" as const },
        formatter: (v: number) => v,
      },
    },
    scales: {
      x: { grid: { color: "rgba(0,0,0,0.05)" }, ticks: { font: { size: 11 }, color: "#5f6368" }, border: { display: false }, beginAtZero: true },
      y: { grid: { display: false }, ticks: { font: { size: 11 }, color: "#5f6368" }, border: { display: false } },
    },
  };
  ...
  const vBarOpts: any = {
    ...
    plugins: {
      ...
      datalabels: {
        ...
        color: "#F15A29", font: { size: 12, weight: "700" as const },
        formatter: (v: number) => v,
      },
    },
    scales: {
  ...
  // Horizontal bar for amounts (INR formatted)
  const amtBarOpts: any = {
    ...hBarOpts,
    plugins: {
      ...hBarOpts.plugins,
      datalabels: {
        anchor: "end", align: "end", offset: 4,
        color: "#1C1C1E", font: { size: 10, weight: "700" as const },
  ...
  const sectionHd: React.CSSProperties = {
    ...
    color: "#9aa0ab", margin: "1.75rem 0 0.75rem", paddingBottom: "0.4rem",
    borderBottom: "1px solid rgba(0,0,0,0.07)",
  };

  const twoCol: React.CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" };

  // ── Fynd Vs. Other Party ─────────────────────────────────────────────────
  const fyndStatusDonut = toDoughnutData(fyndVsOther.statusCounts, [CHART_COLORS.orange, CHART_COLORS.dark, CHART_COLORS.amber, CHART_COLORS.teal]);
  const fyndCauseEntries = toBarData(fyndVsOther.causeCounts);
  const fyndCauseBar = {
    labels: fyndCauseEntries.map(([k]) => k),
    datasets: [{ label: "Cases", data: fyndCauseEntries.map(([, v]) => v), backgroundColor: OB_PALETTE.slice(0, fyndCauseEntries.length), borderRadius: 5, barThickness: 28 }],
  };

  // Aging chart (days since default, per company, coloured by status)
  const agingData = fyndVsOther.companyAging || [];
  const agingBar = {
    labels: agingData.map(d => d.company.length > 22 ? d.company.slice(0, 20) + "…" : d.company),
    datasets: [{
      label: "Aging (days)",
      data: agingData.map(d => d.agingDays),
      backgroundColor: multiColors(agingData.length),
      borderRadius: 5,
      barThickness: 26,
    }],
  };
  const agingBarOpts: any = {
    ...hBarOpts,
    plugins: {
      ...hBarOpts.plugins,
      tooltip: {
        ...
        callbacks: {
          label: (ctx: any) => {
            const d = agingData[ctx.dataIndex];
            return ` ${ctx.raw} days  ·  ${d?.status || ""}`;
          },
        },
      },
      datalabels: {
  ...
  // Net Recoverable Amount per company
  const amtData = fyndVsOther.companyAmounts || [];
  const amtBar = {
    labels: amtData.map(d => d.company.length > 22 ? d.company.slice(0, 20) + "…" : d.company),
    datasets: [{
      label: "Net Recoverable (INR)",
      data: amtData.map(d => d.amountINR),
      backgroundColor: multiColors(amtData.length),
      borderRadius: 5,
      barThickness: 26,
    }],
  };
  ...
  // ── Other Party Vs. Fynd ─────────────────────────────────────────────────
  const otherStatusDonut = toDoughnutData(otherVsFynd.statusCounts, [CHART_COLORS.orange, CHART_COLORS.dark, CHART_COLORS.amber]);
  const otherCauseEntries = toBarData(otherVsFynd.causeCounts);
  const otherCauseBar = {
    labels: otherCauseEntries.map(([k]) => k),
  ...
  const litigationHd: React.CSSProperties = {
    ...
    borderBottom: "2px solid rgba(241,90,41,0.15)",
  };

  return (
    <div>
      {/* ── Litigation Tracker top-level heading ─────────────────────── */}
      <h2 style={litigationHd}>Litigation Tracker</h2>
      <div style={twoCol}>
        <SmallCard title="Case Status" chip={`${fyndVsOther.totalRows} CASES`}>
          <div style={{ height: 260, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Doughnut data={fyndStatusDonut} options={doughnutOpts} />
          </div>
        </SmallCard>
        <SmallCard title="Cause of Action">
          <div style={{ height: 260 }}>
            <Bar data={fyndCauseBar} options={hBarOpts} />
          </div>
        </SmallCard>
      </div>
      <div style={twoCol}>
        <SmallCard title="Aging by Company" chip="DAYS SINCE DEFAULT">
          <div style={{ height: 300 }}>
            {agingData.length > 0
              ? <Bar data={agingBar} options={agingBarOpts} />
              : <div style={{ color: "#9aa0ab", fontSize: "0.9rem", padding: "1rem" }}>No aging data</div>
            }
          </div>
        </SmallCard>
        <SmallCard title="Net Recoverable Amount" chip="INR">
          <div style={{ height: 300 }}>
            {amtData.length > 0
              ? <Bar data={amtBar} options={amtBarOpts} />
              : <div style={{ color: "#9aa0ab", fontSize: "0.9rem", padding: "1rem" }}>No amount data</div>
            }
          </div>
        </SmallCard>
      </div>

      {/* Other Party Vs. Fynd cases — sub-header removed per user request */}
      <div style={twoCol}>
        <SmallCard title="Case Status" chip={`${otherVsFynd.totalRows} CASES`}>
          <div style={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {otherVsFynd.totalRows > 0
              ? <Doughnut data={otherStatusDonut} options={doughnutOpts} />
              : <div style={{ color: "#9aa0ab", fontSize: "0.9rem" }}>No cases on record</div>
            }
          </div>
        </SmallCard>
        <SmallCard title="Cause of Action">
          <div style={{ height: 240 }}>
            {otherCauseEntries.length > 0
...
function DashboardPage({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  const regionBarData = regions.length ? {
    labels: regions,
    datasets: [
      { label: "Open",    data: regions.map(r => pickCnt(r, "Open")),    backgroundColor: "#F15A29", borderRadius: 4, barThickness: 22 },
  ...
  // Doc types bar
  const barPalette = ["#F15A29","#E8472A","#FF7043","#FF8A65","#FFAB91","#1C1C1E"];
  ...
  const docBarOptions: any = {
    ...
    plugins: {
      legend: { display: false },
      tooltip: { backgroundColor: "#202124", titleColor: "#fff", bodyColor: "rgba(255,255,255,0.75)", padding: 10, cornerRadius: 8 },
      datalabels: { anchor: "end", align: "end", offset: 2, color: "#F15A29", font: { size: 13, weight: "700" }, formatter: (v: number) => v },
    },
    scales: {
      x: { grid: { display: false }, ticks: { font: { size: 11 }, color: "#5f6368" }, border: { display: false } },
      y: { grid: { color: "rgba(0,0,0,0.05)" }, ticks: { font: { size: 11 }, color: "#5f6368", maxTicksLimit: 6 }, border: { display: false }, beginAtZero: true },
      ...
      <div className="trk-table-card">
        ...
        <div className="rl-table-header">
          <div className="rl-table-title">
            <i className="fa-solid fa-table-list" style={{marginRight:8,color:"#F15A29"}}></i>
...
const TEAM_META: Record<string, { gradient: string; initials: string; role: string; email: string }> = {
  Farheen:  { gradient: "linear-gradient(135deg,#F15A29,#E8472A)", initials: "FA", role: "Head of Legal",           email: "farheen.ansari@gofynd.com" },
  Aditi:    { gradient: "linear-gradient(135deg,#E8472A,#FF7043)", initials: "AS", role: "Legal Associate",          email: "aditi.sinha@gofynd.com" },
  Samiksha: { gradient: "linear-gradient(135deg,#FF7043,#FF8A65)", initials: "SP", role: "Legal Associate",          email: "samiksha.parekh@gofynd.com" },
  Sreshta:  { gradient: "linear-gradient(135deg,#1C1C1E,#F15A29)", initials: "SR", role: "Legal Associate",          email: "sreshtha@gofynd.com" },
};
    ...
    <div className="lc-pg-content">
      ...
      <div className="team-activity-card">
        ...
        <table className="team-table">
          ...
          <tbody>
            {members.map(m => {
              const meta = TEAM_META[m.member] || { gradient: "linear-gradient(135deg,#F15A29,#E8472A)", initials: m.member.substring(0, 2).toUpperCase(), role: "Legal Team", email: "" };
                ...
                <div className="rl-modal-fgrid">
                  ...
                  <div className="rl-modal-fg" style={{justifyContent:'center'}}>
                    ...
                    <label style={{display:'flex',alignItems:'center',gap:'.5rem',cursor:'pointer',marginTop:'.2rem'}}>
                      <input type="checkbox" checked={form.is_confidential} onChange={e => setF('is_confidential', e.target.checked)} style={{width:'auto',accentColor:'#F15A29'}} />
